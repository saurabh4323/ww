...
export default function RandomVideoCall() {
  ...
  useEffect(() => {
    const loadLibraries = async () => {
      try {
        ...
        setCallStatus("Click Start Video Call to begin");
      } catch (error) {
  ...
  const initializeSocket = (io) => {
    ...
    socketInstance.on("connect", () => {
      ...
      setCallStatus("Connected to server. Click Start Video Call to begin");
    ...
    socketInstance.on("reconnect", (attemptNumber) => {
      ...
      setCallStatus("Reconnected to server. Click Start Video Call to begin");
      setConnectionAttempts(0);
    });
    ...
    socketInstance.on("waiting-for-match", () => {
      setCallStatus("Looking for someone to video chat with...");
      setIsSearching(true);
    });
  ...
  const endCall = () => {
    ...
    setCallStatus("Call ended. Click Start Video Call to begin again");
      ...
      peerRef.current.on("stream", (remoteStream) => {
        ...
        if (remoteVideoRef.current) {
          ...
          remoteVideoRef.current.play().catch((err) => {
            ...
            setCallStatus("Click Play to start video/audio");
  ...
  const startVideoCall = async () => {
    if (!isInitialized || !SimplePeer || !socket || !socket.connected) {
      ...
      console.error("Cannot start video call: initialization incomplete", {
        isInitialized,
    ...
    try {
      ...
      if (
        ...
        throw new Error("No audio or video tracks found in stream");
      }
      ...
      setTimeout(() => {
        setCallStatus("Finding someone to video chat with...");
  ...
  const cancelSearch = () => {
    ...
    setIsSearching(false);
    setIsConnecting(false);
    setCallStatus("Search cancelled. Click Start Video Call to try again.");
          ...
          <div className="bg-gray-800 rounded-lg p-6 mb-6 shadow-lg">
            <div className="text-center">
              <h2 className="text-2xl font-bold mb-4">
                {isInCall ? "You  re in a video call!" : "Start a video call"}
              </h2>
              <p className="text-lg text-gray-300 mb-6">{callStatus}</p>
              ...
              <div className="flex flex-wrap justify-center gap-4">
                {!isInCall && !isConnecting && (
                  <button
                    ...
                    className="bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 disabled:from-gray-500 disabled:to-gray-600 disabled:cursor-not-allowed text-white px-8 py-4 rounded-full font-semibold text-lg flex items-center space-x-2 transform transition hover:scale-105 shadow-lg"
                  >
                    <Video className="w-5 h-5" />
                      ...
                      if (remoteVideoRef.current) {
                        remoteVideoRef.current
                          ...
                          .catch((err) => {
                            console.error("Error playing remote video:", err);
                            setCallStatus(
                              "Failed to play video/audio. Check browser settings."
                            );
                          });
                      }
                    }}
          ...
          <div className="bg-gray-800 rounded-lg p-6 shadow-lg">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm text-gray-300">
              <div className="flex items-start space-x-2">
                ...
                <span>Ensure good lighting for your video</span>
              </div>
              <div className="flex items-start space-x-2">
              ...
              <div className="flex items-start space-x-2">
                ...
                <span>Use Next if the conversation isn t working</span>
              </div>
            </div>
          </div>
        </div>
